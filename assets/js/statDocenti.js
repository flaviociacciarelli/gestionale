fetch("http://localhost:3000/api/docenti/")
  ...
  .then((docenti) => {
    ...
    docenti.forEach((docente) => {
      const genere = docente.infoPersonali.sesso?.toLowerCase();

      if (genere === "m") {
        maschi.push(docente);
      } else if (genere === "f") {
        femmine.push(docente);
      } else {
        altro.push(docente);
      }
    });
    ...
    // Calcoli età medie
    const etaMediaTotale = calcolaEtaMedia(docenti);
    ...
    console.log("Totale Docenti:", docenti.length);
    ...
    console.log("Età media Professoresse:", etaMediaFemmine);
    console.log("Età media Professori maschi:", etaMediaMaschi);
    console.log("Età media Docenti altro:", etaMediaAltro);
    ...
    const configGrafico = {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: 'Età media docenti per genere'
          }
        },
        scales: {
          y: {
...
function calcolaEtaMedia(docenti) {
  ...
  docenti.forEach((docente) => {
    const dataNascitaDoc = docente.infoPersonali.dataNascita;
